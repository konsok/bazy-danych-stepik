...
#     department = request_data.get("department")
#     with driver.session() as session:
#         updated_employee = session.write_transaction(update_employee, employee_id, name, role, department)
...
#                              " WHERE ID(e) = $id RETURN d.name", id=id).single()
#         if result is None:
#             session.write_transaction(delete_employee), session.write_transaction(delete_employee, id)
#             return jsonify({"message": "Employee deleted successfully"}), 200
#         else:
#             department_name = result["d.name"]
#             session.write_transaction(delete_employee, id, department_name)
...
#     if conditions:
#         query += " WHERE " + " AND ".join(conditions)
#         query += " RETURN d.name as name, count(e) as number_of_employees"
#     if sort == "asc":
#         query += " ORDER BY d.name"
#     elif sort == "desc":
#         query += " ORDER BY d.name DESC"
#         results = tx.run(query, name=name).data()
#         departments = [{"name": result["name"], "number_of_employees": result["number_of_employees"]} for result in
#                        results]
#     return departments
#
#
...
#
# def get_employees_by_department(tx, id):
#     query = "MATCH (e:Employee)-[:WORKS_IN]->(d:Department) WHERE ID(d) = $id RETURN e.name, e.role"
#     results = tx.run(query, id=id).data()
#     employees = [{"name": result['e.name'], "role": result['e.role']} for result in results]
#     return employees
#
...
# def get_department_employees(id):
#     with driver.session() as session:
#         result = session.run("MATCH (d:Department) WHERE ID(d) = $id RETURN COUNT(d) as count", id=id).single()
#         if result["count"] == 0:
